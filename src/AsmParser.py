...
class AsmFunction(object):
	...
	def calculate_return_type(self):
		...
		[*] IN CASE OF A FUNCTION RETURNING A NON FLOATING-POINT VALUE:
			...
			RAX/EAX are the return value in x64 calling  convention, thus, by knowing the type put into RAX/EAX, we can also know what the
			return value's type is.
		...
		for inst in instructions:
			if len(inst.operands) > 1 and get_register(inst.operands[0])=='rax':
				src = inst.operands[1]
				return_type = self.get_english_size(src if not is_register(src) else self._reg_state_dict[get_register(src)])
	...
	def _get_value(self, var):
		...
		if is_int(var):
			...
			if var in self._reg_state_dict:
				...
				return self._reg_state_dict[get_register(var)] if get_register(var) in self._reg_state_dict else get_register(var)
		else:
			return self._stack_frame_state[var] if var in self._stack_frame_state else var
	...
	def _set_value(self, var, value):
		if is_register(var):
			...
			self._stack_frame_state[var] = value			
	...
	def get_size(self, value):
		"""
		For the given value, retrn the size of the value (amount of bits).
	...
	def update_state_dicts_by_inst(self, ind, make_c_code=False):
		...
		if inst.operator.startswith('mov'):
			...
			if len(inst.operands) == 1:
				...
				multiplier_value = self._get_value(multiplier)

				if size == 8:
					multiplied_value = self._get_value('al')
					self._set_value('ax', '('+multiplied_value + ')*(' + multiplier_value+')')

				elif size == 16:
					multiplied_value = self._get_value('ax')
					self._set_value('dx', 'HIWORD(('+multiplied_value +')*('+multiplier_value+'))')
					self._set_value('ax', 'LOWORD(('+multiplied_value +')*('+multiplier_value+'))')
				
				elif size == 32:
					multiplied_value = self._get_value('eax')
					self._set_value('edx', 'HIDWORD(('+multiplied_value +')*('+multiplier_value+'))')
					self._set_value('eax', 'LODWORD(('+multiplied_value +')*('+multiplier_value+'))')

				elif size == 64:
					multiplied_value = self._get_value('rax')
					self._set_value('rdx', 'HIQWORD(('+multiplied_value +')*('+multiplier_value+'))')
					self._set_value('rax', 'LOQWORD(('+multiplied_value +')*('+multiplier_value+'))')
			...
			if len(inst.operands) == 1:
				...
				shift_by = self._get_value(power_arg)	
				
			self._set_value(dst, '('+dst_value+')<<('+shift_by+')')
			...
			dst = inst.operands[0]
			self._set_value(dst, '~('+self._get_value(dst)+')')
